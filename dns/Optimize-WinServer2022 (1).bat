...
:: ── 读取本文件 #> 之后的 PowerShell 内容，写入临时 .ps1 执行 ─
:: 原理：Get-Content 读取本 .bat 文件自身，按 #> 分割取第二段
::       写入系统临时目录的 .ps1 文件，执行完毕后删除
echo Running optimization script, please wait...
powershell -NoProfile -ExecutionPolicy Bypass -Command "& { $f = Get-Content '%~f0' -Raw; $ps = ($f -split '#>',2)[1]; $t = [IO.Path]::ChangeExtension([IO.Path]::GetTempFileName(), '.ps1'); [IO.File]::WriteAllText($t, $ps, [Text.Encoding]::UTF8); & $t; Remove-Item $t -Force }"
echo.
...
# ============================================================
# Windows Server 2022 Azure 低配优化 + 简体中文切换
# 适用：2核1G，Server 2022 Datacenter（英文原版）
# 双击本 .bat 文件即可运行，会自动请求管理员权限
# ============================================================
...
# 方式A：Install-Language（Server 2022 推荐）
...
# 方式B：DISM Add-Capability（不依赖版本号，联网拉取，进度可见）
if (-not $installed) {
    try {
...
# 原理：上述设置只影响当前登录用户，欢迎屏幕是独立系统账户，必须单独传播
Write-Host "`n[步骤4] 将语言设置复制到欢迎屏幕和新用户..." -ForegroundColor Yellow

# 方式A：官方 cmdlet（Server 2022 首选）
